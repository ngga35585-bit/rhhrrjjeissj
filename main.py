...
# Variabile de mediu pentru Render
...
async def send_report(session, semaphore, stop_event):
    async with semaphore:
        # Delay mic pentru a simula utilizator real
        ...
        # URL CORECTAT EXPLICIT: v10, @me (nu amе)
        ...
        try:
            async with session.post(url, headers=headers, json=payload) as resp:
                ...
                if status == 200:
                    ...
                    logger.error(f"[404] Endpoint nu găsit. URL: {url}")
                    logger.error(f"[404] Body: {body}")
                    stop_event.set()
                elif status == 403:
...
async def run_reporter(stop_event):
    ...
    async with aiohttp.ClientSession(connector=connector) as session:
        tasks = [asyncio.create_task(send_report(session, semaphore, stop_event)) for _ in range(15)]
